...
# Si el usuario es el dueño original, le asigno rol de 'admin'.
# Si no es el dueño, busco en la tabla de miembros (ProjectMember) qué rol tiene asignado.
# Si no pertenece al proyecto, devuelvo None.
...
def project_list_view(request):
    # Busco proyectos donde el usuario sea dueño o aparezca en la lista de colaboradores
...
def project_create_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Creo el proyecto pero no lo guardo en la BD todavía
            project = form.save(commit=False)
            # Asigno al creador como dueño del proyecto
            project.owner = request.user
            project.save()
            
            # 1. Asigno al creador como miembro Administrador (rol principal)
            ProjectMember.objects.create(project=project, user=request.user, role='admin')
            
            # 2. Asigno los colaboradores iniciales agregados dinámicamente desde el buscador
            ...
            # 3. Creo las 3 columnas obligatorias por defecto del tablero Kanban
            ...
            # Redirijo al usuario al tablero del proyecto recién creado
...
def project_detail_view(request, project_id):
    # Obtengo el proyecto o devuelvo un error 404 si no existe
    project = get_object_or_404(Project, id=project_id)
    
    # Compruebo el rol del usuario actual
    role = get_user_role(project, request.user)
    if not role:
        return HttpResponseForbidden("No tienes acceso a este proyecto.")

    # Obtengo todas las columnas y los mensajes del chat del proyecto
    columns = project.columns.all()
    messages = project.messages.all()

    # Preparo las etiquetas y cantidades de tareas por columna para pintar la gráfica
    chart_labels = []
    chart_counts = []
    for col in columns:
    ...
    # Renderizo la plantilla pasando los datos del proyecto, roles, columnas y el json de la gráfica
...
def project_members_view(request, project_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Creo la membresía o la actualizo si el usuario ya tenía un rol asignado
...
def update_member_role_view(request, project_id, member_id):
    ...
    # Impido que el administrador se modifique o degrade su propio rol de creador
    if member.user == project.owner:
        return HttpResponseForbidden("No puedes cambiar el rol del propietario del proyecto.")

    if request.method == 'POST':
        new_role = request.POST.get('role')
        # Valido que el rol esté dentro de las opciones válidas definidas en el modelo
...
def update_task_column_api(request):
    if request.method == 'POST':
        try:
            # Parseo el JSON recibido desde el cuerpo de la petición
            ...
            # Obtengo la columna destino del mismo proyecto y reasigno la tarea
...
def task_create_view(request, project_id):
    ...
    if request.method == 'POST':
        # Le paso el proyecto al formulario para que filtre asignados y columnas
...
def task_edit_view(request, task_id):
    ...
    # Compruebo si el usuario es un desarrollador (developer) asignado a esta tarea específica
    is_developer_assigned = (role == 'developer' and request.user in task.assigned_to.all())

    # Si no es admin, ni manager, ni el desarrollador asignado, deniego el acceso
    if role not in ['admin', 'manager'] and not is_developer_assigned:
    ...
    if request.method == 'POST':
        ...
        if is_developer_assigned:
            # Deshabilito en el formulario de desarrolladores todos los campos excepto el de columna
